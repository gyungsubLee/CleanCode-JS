...
초기화 이전에 변수에 접근하려고 하면 참조 에러(ReferenceError)가 발생하며 이는 변수가 아직 초기화되지 않았기 때문이다.<br/>
 즉, 변수를 위한 메모리 공간이 아직 확보되지 않았기 때문이다.<br/> 따라서 스코프의 시작 지점부터 초기화 시작 지점까지는 변수를 참조할 수 없다. <br/>그리고 스코프의 시작 지점부터 초기화 시작 지점까지의 구간을 ‘일시적 사각지대(Temporal Dead Zone; TDZ)’라고 부른다.
